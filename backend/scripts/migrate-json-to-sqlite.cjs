/**
 * One-time / CI: apply db/schema.sql and import data/users.json + db/pmu-course-catalog.json
...
const catalogPath = path.join(root, 'db', 'pmu-course-catalog.json')
...
const demoAcademicPath = path.join(root, 'db', 'demo-academic.sql')
...
if (sectionsAfter === 0 && fs.existsSync(demoAcademicPath)) {
  ...
  console.log('Applied db/demo-academic.sql (sections, grades, registrations, settings).')
}
